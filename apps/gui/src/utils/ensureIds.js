...
export function ensureEnumIds(enums) {
  ...
  return (enums ?? []).map((e) => (typeof e.id === "string" ? e : { ...e, id: nanoid() }));
}
...
export function ensureTypeIds(types) {
  return (types ?? []).map((t) => {
    ...
    if (typeof t.id === "string" && fields.every((f) => typeof f.id === "string")) return t;
    return {
      ...t,
      id: typeof t.id === "string" ? t.id : nanoid(),
      fields: fields.map((f) => (typeof f.id === "string" ? f : { ...f, id: nanoid() })),
    };
  });
}
