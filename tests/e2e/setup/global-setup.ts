...
async function spawnBrowserKeeper(wsEndpoint: string, guiUrl: string): Promise<KeeperHandle> {
...
export default async function globalSetup(): Promise<void> {
  ...
  process.stderr.write(`[e2e] container started: gui=${running.guiUrl} mcp=${running.mcpUrl}\n`);
  ...
  try {
    ...
    if (browserServerPid !== null) {
      try {
        process.kill(browserServerPid, "SIGTERM");
      } catch {}
    }
